...
export default function AnnonceDetailsPage() {
  ...
  if (loading) {
    return <div className="shell-loader">Chargement de l'annonce...</div>;
  }

  if (!post) {
    return <div className="shell-loader">Annonce introuvable</div>;
  }
      ...
      <div className="panel" style={{ overflow: 'hidden' }}>
        ...
        <div style={{ padding: '2rem' }}>
          ...
          <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap', marginBottom: '2rem', padding: '1rem', backgroundColor: '#f8fafc', borderRadius: '0.5rem' }}>
            <div>
              <p className="muted-copy" style={{ fontSize: '0.875rem' }}>Prix estimatif</p>
              <strong style={{ fontSize: '1.25rem' }}>{priceLabel}</strong>
            </div>
            <div>
              <p className="muted-copy" style={{ fontSize: '0.875rem' }}>Disponibilité</p>
              <strong style={{ fontSize: '1.25rem' }}>{post.available_at ? formatDateTime(post.available_at) : 'Dès que possible'}</strong>
            </div>
          </div>

          <div style={{ marginBottom: '3rem' }}>
            <h3 style={{ marginBottom: '1rem' }}>Description du service</h3>
          ...
          <div>
            <h3 style={{ marginBottom: '1rem' }}>À propos de l'artisan</h3>
